...
6. 파드 running
~~~
kubectl apply : 쿠버네티스에 적용하고 싶은 오브젝트의 명세를 전달하는 명령어, 파드를 생성하거나 생성한 파드를 명세에 맞게 수정한다. </br> </br>
파드 재시작 정책 (restartPolicy)
  - Always
  - OnFailure
  - Never
...
→ 디플로이먼트 : 파드의 명세가 바뀌는 경우를 지원하기위한 오브젝트 </br>
...
컨테이너 이미지는 불변성을 가지는것이 원칙. 따라서 업데이트와 같은 변경이 생겼을때 새로운 이미지를 만드는 방법 사용한다. </br>
이때, 기존이미지와 새로운 이미지의 구분을 위해 tag사용한다.
...
전략 
- recreate전략 : 기본파드를 모두 종료한뒤 새 버전의 파드실행 
...
- 롤링업데이트 전략을 사용했음에도 무중단 업데이트가 실패하는 경우
  ...
  → 종료절차에 들어간 파드는 이미 들어온 요청에대해 응답을 모두 보낸뒤 종료 : graceful shot down </br>
  → 새로 실행된 파드는 요청을 받아들인 준비가 되었을때부부터 요청을 받기 시작 : Readiness probe
...
히스토리를 보면, 롤백된 디플로이먼트가 기존 이력에서 삭제되고, 새로운 이력으로 추가된다.
